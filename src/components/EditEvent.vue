    ...
    <div class="form-card">
      ...
      <form @submit.prevent="handleSubmit" class="event-form">
        ...
        <div class="form-actions">
          <button type="button" @click="handleDelete" class="delete-btn">删除活动</button>
...
const loadEvent = async () => {
  try {
    ...
    formData.start_time = event.start_time.replace(' ', 'T').slice(0, 16);
    formData.end_time = event.end_time.replace(' ', 'T').slice(0, 16);
    formData.location = event.location;
    formData.organizer = event.organizer;
    formData.max_participants = event.max_participants;
  } catch (error) {
